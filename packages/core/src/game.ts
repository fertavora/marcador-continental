...
// Not globally available on RN's Hermes runtime (unlike web/Node), so this
// avoids depending on `generateId()` for an ID that only needs to be
// unique, not cryptographically secure.
