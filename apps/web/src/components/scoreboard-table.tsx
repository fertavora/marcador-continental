...
import { getTotals, ROUND_NAMES, type Game } from "@marcador/core"
...
export function ScoreboardTable({ game }: { game: Game }) {
  ...
  return (
    <div className="overflow-x-auto rounded-md border" data-testid="scoreboard-table">
      <Table className="table-fixed">
        ...
        <TableBody>
          {game.rounds.map((round, roundIndex) => (
            <TableRow key={roundIndex} data-testid={`scoreboard-round-${roundIndex + 1}`}>
              <TableCell
              ...
              {sortedPlayers.map((player) => (
                <TableCell
                  ...
                  data-testid={`scoreboard-round-${roundIndex + 1}-score-${player.id}`}
                >
          ...
          <TableRow className="border-t-2">
            ...
            {sortedPlayers.map((player) => (
              <TableCell
                ...
                data-testid={`scoreboard-total-${player.id}`}
              >
                {totals[player.id] ?? 0}
              </TableCell>
            ))}
